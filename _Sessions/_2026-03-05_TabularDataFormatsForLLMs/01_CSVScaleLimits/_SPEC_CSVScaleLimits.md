...
**Target files**: `01_CSVScaleLimits/_Scripts/*.py`, `01_CSVScaleLimits/test-config-template.json`
...
Creates `scale_limit_result.json` in model output folder:
...
```json
{
  ...
  "context_utilization_at_failure_pct": 78.5,
  "max_context_tokens": 128000,
  "total_cost_usd": 0.045123,
  "search_history": [
    {
      "rows": 500, "precision": 0.85, "recall": 0.90, "passed": false,
      "input_tokens": 45000, "output_tokens": 8000, "finish_reason": "length",
      "truncated": true, "failure_mode": "truncation", "cost_usd": 0.012
    }
  ],
  "total_iterations": 6,
  "elapsed_time_seconds": 245.3,
  "timing_stats": {"min_secs": 12.5, "max_secs": 45.2, "avg_secs": 28.1}
}
```
...
**TBLF-FR-07: Batch Testing Across Models and Configurations**

Automated runner to test multiple models and effort levels for hypothesis testing.
...
```bash
python 04_batch_scale_test.py --test-path .. --config batch-config.json
```

**Arguments:**
- `--test-path` - Path to test folder
- `--config` - Batch configuration file (default: `batch-config.json`)
...
```json
{
  "test_matrix": [
    ...
    {"model": "gpt-4o-mini", "reasoning_effort": "medium"},
    {"model": "claude-sonnet-4-20250514", "reasoning_effort": "medium"}
  ],
  "initial_rows": 100,
  "tolerance": 10
}
...
Creates one result folder per configuration:
```
01_CSVScaleLimits/
├── gpt-5-mini_reasoning_effort_low_max16384/
│   └── scale_limit_result.json
├── gpt-5-mini_reasoning_effort_medium_max16384/
│   └── scale_limit_result.json
└── batch_summary.json
```
...
```json
{
  ...
  "results": [
    {"model": "gpt-5-mini", "effort": "low", "scale_limit": 280, "failure_mode": "truncation"},
    {"model": "gpt-5-mini", "effort": "medium", "scale_limit": 340, "failure_mode": "truncation"},
    {"model": "gpt-5-mini", "effort": "high", "scale_limit": 380, "failure_mode": "comprehension"}
  ],
  "total_cost_usd": 0.25,
...
**TBLF-FR-08: Hypothesis Testing and Report Generation**

Analyzes `scale_limit_result.json` files to test hypotheses and generate comparison reports.
...
**Arguments:**
- `--test-path` - Path to test folder containing result folders
- `--output` - Output file for analysis report (default: `analysis_report.md`)
...
├─────────────────────────────────────────────────────────────────┤
│ 1. Scan test folder for */scale_limit_result.json              │
│ 2. Load all result files into memory                            │
│ 3. For each hypothesis:                                         │
│    - Extract relevant metrics                                   │
│    - Calculate statistics (mean, std, correlation)             │
│    - Determine SUPPORTED / NOT SUPPORTED / INCONCLUSIVE        │
...
- [ ] `04_batch_scale_test.py` runs scale test for each config in test_matrix
- [ ] `04_batch_scale_test.py` generates batch_summary.json
- [ ] `05_analyze_results.py` scans for scale_limit_result.json files
- [ ] `05_analyze_results.py` tests H1-H5 hypotheses with statistics
- [ ] `05_analyze_results.py` generates markdown report
...
- Updated: Verification checklist with new script requirements

**[2026-03-05 22:46]**
- Added: EvaluationResult fields - truncated, finish_reason, input_tokens, output_tokens
- Added: scale_limit_result.json fields - provider, method, primary_failure_mode, context_utilization_at_failure_pct, total_cost_usd
- Fixed: V1 folder references updated to current folder name
- Implements: TBLF-RV-001, TBLF-RV-004, TBLF-RV-006, TBLF-RV-007
